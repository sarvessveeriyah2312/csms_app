...
import 'package:csms_app/Data%20Layer/Model/Admin%20Model.dart';
...
class Db {
  ...
  Future<MOHModel> getSpecificMOHData(int value) async {
    Database db = await instance.database;
    final adminData = await db.query('MOH',
        columns: ['id', 'name', 'age'], where: 'id = ?', whereArgs: ['id']);

    if (adminData.isNotEmpty) {
      return MOHModel.fromJson(adminData.first);
    }
  ...
  //get all MOH data
  Future<List<MOHModel>> getAllMOHData() async {
    Database db = await instance.database;
    final adminModel = await db.query('MOH', orderBy: 'name');
    return adminModel.map((json) => MOHModel.fromJson(json)).toList();
  }
}
